  ...
  const callChatAPI = async (question: string): Promise<string> => {
    try {
      const response = await fetch("http://103.225.98.179:5000/api/chat/ask", {
        ...
        body: JSON.stringify({ question }), // cukup singkat
      });
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const botMsg: Message = {
        ...
        from: "bot",
        // text:
        //   apiResponse.similarity_score && apiResponse.similarity_score > 0.1
        //     ? apiResponse.answer
        //     : "Maaf, saya belum punya jawaban untuk itu.",
      ...
      // Update chat history jika ada active chat
      if (activeChatId) {
        setChatHistory((prev) =>
          prev.map((chat) =>
            chat.id === activeChatId
