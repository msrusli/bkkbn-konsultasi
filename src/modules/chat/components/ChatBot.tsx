...
// import React, { useState, useRef, useEffect } from "react";
// import chatData from "../data/chatbot.json"; // Pastikan path ini benar
...
// // --- FUNGSI TF-IDF & COSINE SIMILARITY ---
...
// // 2. TF-IDF: Menghitung Term Frequency
// const calculateTF = (document: string[]): { [key: string]: number } => {
...
// // 3. IDF: Menghitung Inverse Document Frequency
// const calculateIDF = (
//   documents: string[][],
...
//     const documentCount = documents.filter((doc) => doc.includes(word)).length;
//     idf[word] = Math.log(N / (documentCount + 1));
//   });
...
// // 5. Membuat vektor TF-IDF dari dokumen
// const createTFIDFVectors = (
//   documents: string[][]
// ): {
//   vectors: number[][];
//   vocabulary: string[];
...
// } => {
//   const vocabulary = Array.from(new Set(documents.flat())).sort();
//   const idf = calculateIDF(documents, new Set(vocabulary));

//   const vectors = documents.map((doc) => {
//     const tf = calculateTF(doc);
//     return vocabulary.map((word) => (tf[word] || 0) * idf[word]);
//   });
...
//   useEffect(() => {
//     // Pre-processing dataset menjadi vektor TF-IDF
//     const questions = chatData.map((qa) => tokenize(qa.question));
//     const processedData = createTFIDFVectors(questions);
//     setDatasetVectors(processedData);
//   }, []);
...
//   const sendMessage = () => {
//     if (!input.trim() || !persona || !datasetVectors) return;
...
//     // Tokenisasi dan vektorisasi pertanyaan pengguna
//     const userTokens = tokenize(input);
//     const userTF = calculateTF(userTokens);
//     const userVector = datasetVectors.vocabulary.map(
//       (word) => (userTF[word] || 0) * datasetVectors.idf[word]
//     );

//     let bestAnswer = "Maaf, saya belum punya jawaban untuk itu.";
//     let highestScore = 0;

//     // Cari jawaban terbaik menggunakan Cosine Similarity
//     datasetVectors.vectors.forEach((vector, index) => {
//       const score = calculateCosineSimilarity(userVector, vector);
//       if (score > highestScore) {
//         highestScore = score;
//         bestAnswer = chatData[index].answer;
//       }
//     });
...
//           : "Maaf, saya belum punya jawaban untuk itu.",
//       timestamp: getCurrentTime(),
//       containsHTML: highestScore > 0.1 && hasHTML,
//     };
...
//       const answer: string = result.answer ?? "Jawaban tidak ditemukan";
...
//         // Update chat history setelah animasi selesai
//         if (activeChatId) {
//           setChatHistory((prev) =>
//             prev.map((chat) =>
//               chat.id === activeChatId
...
//       const apiResponse = await callChatAPI(input);
//       const hasHTML = containsHTML(apiResponse);

//       // Buat pesan bot dengan text kosong untuk animasi
//       const botMsg: Message = {
//         id: crypto.randomUUID(),
//         from: "bot",
//         text: "", // Mulai dengan text kosong
//         timestamp: getCurrentTime(),
//         containsHTML: hasHTML,
//         isTyping: true,
//       };

//       // Tambahkan pesan bot ke state
//       setMessages((prev) => [...prev, botMsg]);

//       // Update chat history sementara tanpa animasi
//       if (activeChatId) {
//         setChatHistory((prev) =>
//           prev.map((chat) =>
//             chat.id === activeChatId
...
//       startTypingAnimation(botMsg.id, apiResponse, 30);
//     } catch (error) {
...
//   // Komponen untuk merender pesan dengan animasi
//   const MessageContent = ({ message }: { message: Message }) => {
//     if (message.containsHTML) {
//       return (
//         <div
...
//   const [isListening, setIsListening] = useState(false);
//   const bottomRef = useRef<HTMLDivElement>(null);
//   const inputRef = useRef<HTMLInputElement>(null); // Ref untuk input text
//   const recognitionRef = useRef<any>(null);
//   const typingIntervalsRef = useRef<{ [key: string]: NodeJS.Timeout }>({});
...
//   // Effect untuk fokus ke input setelah mengirim pesan
//   useEffect(() => {
//     // Fokus ke input ketika tidak sedang mengetik (setelah pesan terkirim)
//     if (!isTyping && inputRef.current) {
//       inputRef.current.focus();
//     }
//   }, [isTyping, messages]); // Trigger ketika status typing berubah atau messages update
...
//                   className="flex-1 p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
//                   disabled={isTyping}
//                   autoFocus // Otomatis fokus ketika komponen dimount
//                 />
...
//   containsHTML?: boolean;
//   isTyping?: boolean;
//   references?: Reference[]; // Update untuk reference object
// }
...
//   // Fungsi untuk memanggil API - DIUPDATE
...
//       const result = await response.json();
//       const answer: string = result.answer ?? "Jawaban tidak ditemukan";
//       const references: Reference[] = result.references || [];
...
//   // Fungsi untuk animasi mengetik - DIUPDATE
//   const startTypingAnimation = (
//     messageId: string,
//     fullText: string,
...
//     try {
//       // Panggil API untuk mendapatkan jawaban - DIUPDATE
//       const apiResponse = await callChatAPI(input);
//       const hasHTML = containsHTML(apiResponse.answer);
...
//       // Mulai animasi mengetik - DIUPDATE
...
//   // Komponen untuk merender references - DIUPDATE
//   const ReferencesSection = ({ references }: { references: Reference[] }) => {
//     if (!references || references.length === 0) return null;

//     return (
//       <div className="mt-3 pt-3 border-t border-gray-200">
//         <div className="text-xs text-gray-500 mb-2 font-medium">Referensi:</div>
...
//                             {/* Tampilkan references hanya untuk pesan bot yang sudah selesai typing */}
//                             {m.from === "bot" &&
//                               !m.isTyping &&
//                               m.references &&
//                               m.references.length > 0 && (
//                                 <ReferencesSection references={m.references} />
//                               )}
//                           </div>
//                         </div>
//                         <div
...
export default function Chatbot(): JSX.Element {
  ...
  // Fungsi untuk mengirim pertanyaan dari reference
  const sendQuestionFromReference = (question: string) => {
    if (!persona) return;

    // Set input dengan pertanyaan dari reference
    setInput(question);
    ...
    setTimeout(() => {
      if (inputRef.current) {
        ...
        // Trigger send message setelah delay singkat
        setTimeout(() => {
          sendMessage();
        }, 100);
      }
      ...
      <div className="mt-3 pt-3 border-t border-gray-200">
        ...
        <div className="space-y-2">
          {references.map((ref, index) => (
            <div
              key={index}
              className="text-xs text-gray-600 bg-gray-50 p-2 rounded-lg hover:bg-blue-50 hover:border-blue-200 border border-transparent transition-all duration-200 cursor-pointer group"
              onClick={() => sendQuestionFromReference(ref.title)}
            >
              <div className="font-medium text-blue-700 group-hover:text-blue-800 flex items-center">
                {ref.title}
                <svg
        ...
        <div className="text-xs text-gray-400 mt-2 italic">
          Klik judul referensi untuk menanyakan lebih lanjut
        </div>
      </div>
    );
  };
